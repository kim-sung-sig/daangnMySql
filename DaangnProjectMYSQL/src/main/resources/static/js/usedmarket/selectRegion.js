$(function(){
	...
	regionSelect.on('change', function(){
		...
		if(selectedRegion == '0'){
			location.href = `/used-market/view` + query;					
		} else {
			location.href = `/used-market/view/${selectedRegion}` + query;					
		}
	});
	...
	if(getRegion.length != 0){
		...
		guSelect.on('change', function(){
			...
			if(selectedGu == '0'){
				location.href = `/used-market/view/${getRegion}` + query;					
			} else {
				location.href = `/used-market/view/${getRegion}/${selectedGu}` + query;					
			}
		})
	}
	...
	if(getGu.length != 0){
		...
		dongSelect.on('change', function(){
			...
			if(selectedDong == '0'){
				location.href = `/used-market/view/${getRegion}/${getGu}` + query;					
			} else {
				location.href = `/used-market/view/${getRegion}/${getGu}/${selectedDong}` + query;					
			}
		})
	}
