...
SELECT
    6,
    (seq.seq % 13) + 1,
    'bbe64d63-c6fa-4927-849c-9eec89c52975_다운로드 (18).jfif',
    CONCAT('제목 ', seq.seq),
    CONCAT('내용 ', seq.seq),
    10000,
    37.54093868750438, 
...
FROM (
    SELECT 
        (a.num + b.num * 10 + c.num * 100) AS seq
    FROM 
    ...
    WHERE 
        (a.num + b.num * 10 + c.num * 100) + 1 <= 1000
    ORDER BY 
    	seq
) seq
...
SELECT 
    dlb.idx, 
    CONCAT('bbe64d63-c6fa-4927-849c-9eec89c52975_다운로드 (18).jfif')
FROM 
